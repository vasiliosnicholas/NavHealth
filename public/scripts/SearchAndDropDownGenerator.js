 ...
 * @param {*} onCategoryChange optional callback when the active category changes.
...
export default function SearchAndDropDownGenerator(
  ...
  const categoryChangeListeners = [gen.populateDataList];
  if (onCategoryChange) {
    categoryChangeListeners.push(onCategoryChange);
  }
  ...
  function getActiveSearchCategory() {
    return dropDownGen.getActiveSearchCategory();
  }
