...
function renderResultCard(location, index) {
  ...
  const ratingBadge =
    !isAdminMode && reviewsMetaData
      ? `<a class="result-badge align-items-end" style="text-decoration:none" href="${reviewsFullUrl}">Rating: ${parseFloat(reviewsMetaData.average_rating).toFixed(FLOAT_PRECISION)} <small> from ${reviewsMetaData.num_reviews} reviews</small></a>`
      : "";
